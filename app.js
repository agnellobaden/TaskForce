...
// This ensures that pressing "Back" on mobile closes overlays/modals instead of exiting the app.
document.addEventListener('DOMContentLoaded', () => {
...
function setupBackButtonLogic() {
    ...
    // List of Modals/Sections that should block "Exit" and be closable via Back Button
    // Includes Overlays, Modals, and Full-Screen Sections
    const managedViews = [
        'urgentOverlay',
        'teamNotificationOverlay',
        'driveModeOverlay',
        'nightstandOverlay',
        'activeAlarmOverlay',
        'sideMenuOverlay',
        ...
        'calendarModal',
        'appointmentModal',
        'aiResearchResultModal',
        'alarmSettingsModal',
        'installBanner',

        // Full screen sections/views that act like pages
        'expenseSection',
        'alarmSection',
        'quickTodoSection',
        'aiAdvisorSection'
    ...
    // Helper: Find which view is currently open
    // We assume "open" means not having 'hidden' class
    const getOpenViews = () => {
        return managedViews
    ...
    // 1. Listen for PopState (User presses Back)
    window.addEventListener('popstate', (event) => {
        // Find visible managed views
        const openViews = getOpenViews();

        if (openViews.length > 0) {
            // User pressed back, hoping to close the top view
            isHandlingBack = true;

            // Close the detected open views -> acts as "Back" navigation
            openViews.forEach(view => {
                view.classList.add('hidden');
                console.log(`[Back Handler] Closing view: ${view.id}`);
            });

            // Allow flag to reset
            setTimeout(() => { isHandlingBack = false; }, 50);
        } else {
            // No views open. Let browser handle history (Exit or prev page).
        }
    });

    // 2. Observe View Openings (Push State)
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                ...
                if (!isHidden) {
                    // View Opened
                    if (!isHandlingBack) {
                        // Push a state so "Back" has something to pop
                        // We check if we are already in a "modal" state to avoid stacking too many?
                        // For now, simple stacking is intuitive: Back closes last opened.
                        history.pushState({ viewId: target.id, timestamp: Date.now() }, '', '#' + target.id);
                    }
                } else {
                    // View Closed
                    // If closed by UI (Close Button) -> we are still deep in history stack.
                    // We must pop back to base state to keep stack clean and avoid "Double Back" needed later.
                    if (!isHandlingBack) {
                        // Only go back if we likely pushed a state for this.
                        // If stack is empty (null state), maybe we shouldn't back?
                        // But if we just opened it, state should be there.
                        if (history.state) {
                            history.back();
                        }
                    }
    ...
    managedViews.forEach(id => {
        ...
        if (el) {
            ...
            // console.warn(`[Back Handler] Managed view not found: ${id}`);
        }
    });
}
